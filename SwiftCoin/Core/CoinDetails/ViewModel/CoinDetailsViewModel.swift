...
class CoinDetailsViewModel {
    ...
    var overViewSectionModel: CoinDetailSectionModel {
        ...
        let currentPriceStat = StatisticModel(title: "Current Price", value: price, percentChange: pricePercentChange)
        
        let marketCap = coin.marketCap ?? 0
        let marketCapPercentChange = coin.marketCapChangePercentage24H
        let marketCapStat = StatisticModel(title: "Market Capitalization", value: marketCap.formattedWithAbbreviations(), percentChange: marketCapPercentChange)
        
        let rank = coin.marketCapRank
        let rankStat = StatisticModel(title: "Rank", value: "\(rank)", percentChange: nil)
        
        let volume = coin.totalVolume ?? 0
        let volumeStat = StatisticModel(title: "Volume", value: volume.formattedWithAbbreviations(), percentChange: nil)
        
        return CoinDetailSectionModel(title: "Overview", statistics: [currentPriceStat, marketCapStat, rankStat, volumeStat])
    }
    
    var additionalDetailSectionModel: CoinDetailSectionModel {
        let high = coin.high24H?.toCurrency() ?? "n/a"
        let highStat = StatisticModel(title: "24H High", value: high, percentChange: nil)
        
        let low = coin.low24H?.toCurrency() ?? "n/a"
        let lowStat = StatisticModel(title: "24H Low", value: low, percentChange: nil)
        
        let priceChange24H = coin.priceChange24H.toCurrency()
        let percentChange24H = coin.priceChangePercentage24H
        let priceChangeStat = StatisticModel(title: "24H Price Change", value: priceChange24H, percentChange: percentChange24H)
        
        let marketCapChange = coin.marketCapChange24H ?? 0
        let marketCapChangePercent = coin.marketCapChangePercentage24H
        let marketCapChangeStat = StatisticModel(title: "24 H Market Cap Change", value: "$\(marketCapChange.formattedWithAbbreviations())", percentChange: marketCapChangePercent)
        
        return CoinDetailSectionModel(title: "Additional Details", statistics: [highStat, lowStat, priceChangeStat, marketCapChangeStat])
    }
